...
const Post = ({ post, exchangeRate, profile, nodes, isRepost }) => {
    ...
    return (
        <div className={`${isRepost ? `my-2  rounded-xl ` : ``} flex bg-white border ${!isRepost ? `hover:bg-gray-50/80` : `hover:bg-gray-100/50`} transition duration-100 justify-center items-center mx-auto border-gray-200 p-[15px] w-full flex-row max-w-[500px]`}>
            <div className='w-full flex-1' onClick={() => handleClick(`${nodeURL}/posts/${post.PostHashHex}`)} rel="noopener noreferrer nofollow" target='_blank' style={{ cursor: 'pointer'}}>
